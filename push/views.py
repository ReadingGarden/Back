...
def send_book_push(request):
    ...
    """
    logger.info(f"Call send_book_push API")
    return RETURN_FUNC(push_service.send_book_push())

@router.post("/notice",
            response={200: DataResp, 400: HttpResp, 500: HttpResp}, 
            summary="푸시 알림 전송 (공지사항 알림)")
def send_notice_push(request, content: str):
    """
    FCM 멀티캐스트 푸시 알림 보내기 (공지사항 알림)
    """
    logger.info(f"Call send_notice_push API")
    return RETURN_FUNC(push_service.send_notice_push(content))
